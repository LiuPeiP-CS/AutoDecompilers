...
abs_path = "/workspace/trl-main/temp/temp"
...
class OverallRewards:
    ...
    def get_code_rewards(self, dependency, response, ground_truth):
        ...
        def _do_heavy_rewards():
            ...
            pre_code = self.remove_special_tokens(pre_code)
            gt_code = self.remove_special_tokens(ground_truth)

            if len(pre_code) > len(gt_code) * 2.0:
            ...
            try:
                if self.re_exe_reward:
                    ...
                    with Re_exe(decom=response, dependencies=dependency, tmp_path=abs_path) as recom_reexe:
        ...
        try:
            ...
            final_reward, final_feedback = func_timeout(TIMEOUT_SECONDS, _do_heavy_rewards)
            return final_reward, final_feedback

        except FunctionTimedOut:
            logger.error(
                f"[get_code_rewards] 🚨 Hard timeout triggered after {TIMEOUT_SECONDS}s! Process safely aborted to save thread.")
            # 返回兜底奖励，确保外部流程继续进行
            return [0, 0, 0, 0], "Code execution or parsing took too long. Please regenerate a simpler code."
